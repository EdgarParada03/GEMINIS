...
public class EditarBaseDatos {
    ...
    public void editarProgramaAcademico(String tableName, String nombrePrograma, String descripcion, String duracion, String nivelAcademico, String facultad, float tarifaBase, int id) throws SQLException {
    try (Connection connection = DriverManager.getConnection(DATABASE_URL, DATABASE_USER, DATABASE_PASSWORD)) {
        String query = "UPDATE " + tableName + " SET nombrePrograma = ?, descripcion = ?, duracion = ?, nivelAcademico = ?, facultad = ?, tarifaBase = ? WHERE idProgramaAcademico = ?";
        PreparedStatement ps = connection.prepareStatement(query);
    ...
    try (Connection connection = DriverManager.getConnection(DATABASE_URL, DATABASE_USER, DATABASE_PASSWORD)) {
        String query = "UPDATE estudiantes SET nombre = ?, apellido = ?, edad = ?, usuario = ?, password = ?, idCurso = ?, idAsignatura = ? WHERE usuario = ?";
        PreparedStatement ps = connection.prepareStatement(query);
        ...
        ps.setInt(6, estudiante.getIdCurso());
        ps.setInt(7, estudiante.getIdAsignatura());
        ps.setString(8, estudiante.getUsuario()); // Usamos el usuario como identificador único para encontrar el estudiante a editar
        ps.executeUpdate();
    }
}
